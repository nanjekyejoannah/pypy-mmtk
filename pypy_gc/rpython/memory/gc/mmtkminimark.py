...
ll_mmtk_alloc = llexternal_mmtk('alloc', [rffi.VOIDP, rffi.SIGNED, rffi.SIGNED, rffi.SIGNED], rffi.VOIDP)
# ll_mmtk_alloc = llexternal_mmtk('alloc', [rffi.SIGNED, rffi.SIGNED, rffi.SIGNED], rffi.VOIDP)
ll_mmtk_post_alloc = llexternal_mmtk('mmtk_post_alloc', [rffi.VOIDP, rffi.VOIDP, rffi.SIGNED], lltype.Void)
...
class MMTKMiniMarkGC(MovingGCBase):
    _alloc_flavor_ = "raw"
    ...
    def setup(self):
        ...
        ll_mmtk_gc_init()
        if ll_mmtk_is_gc_initialized() is True:
            debug_print("the GC is intialised")
        else:
            debug_print("MMTK Not Intialized")
        self.mmtk_handle = ll_mmtk_bind_mutator()
    ...
    def malloc_fixedsize_clear(self, typeid, size,
        ...
        if needs_finalizer and not is_finalizer_light:
            ...
            obj = self.mmtk_external_malloc(typeid, 0, alloc_young=False)
            res = llmemory.cast_adr_to_ptr(obj, llmemory.GCREF)
            ll_mmtk_add_finalizer(res)
        ...
        if rawtotalsize > self.nonlarge_max:
            ...
            obj = self.mmtk_external_malloc(typeid, 0, alloc_young=True)
            #
        else:
    ...
    def malloc_varsize_clear(self, typeid, length, size, itemsize,
        ...
        if r_uint(length) >= r_uint(toobig):
            ...
            obj = self.mmtk_external_malloc(typeid, length, alloc_young=True)
            #
        else:
    ...
    def mmtk_external_malloc(self, typeid, length, alloc_young):
            """Allocate a large object using the ArenaCollection or
            raw_malloc(), possibly as an object with card marking enabled,
            if it has gc pointers in its var-sized part.  'length' should be
            specified as 0 if the object is not varsized.  The returned
            ...
            if (raw_malloc_usage(totalsize) <= self.small_request_threshold
                ...
                # Allocate from the ArenaCollection and clear the memory returned.
                ptr = ll_mmtk_alloc(self.mmtk_handle, totalsize, 8, size_gc_header)
                ll_mmtk_post_alloc(self.mmtk_handle,ptr, totalsize)
                result = llmemory.cast_ptr_to_adr(ptr)
                llmemory.raw_memclear(result, totalsize)
                #
                extra_flags = GCFLAG_TRACK_YOUNG_PTRS
                #
                ...
                # flexibility of saying that we have extra words in the header.
                # The memory returned is cleared by a raw_memclear().
                ptr = ll_mmtk_alloc(self.mmtk_handle, allocsize, 8, size_gc_header)
                ll_mmtk_post_alloc(self.mmtk_handle,ptr, allocsize)
                arena = llmemory.cast_ptr_to_adr(ptr)
                # arena = llarena.arena_malloc(allocsize, 2)
                if not arena:
                    raise MemoryError("cannot allocate large object")
                #
    ...
    def collect_roots_in_nursery(self):
        ...
        self.root_walker.walk_roots(
            MMTKMiniMarkGC._trace_drag_out1,  # stack roots
            MMTKMiniMarkGC._trace_drag_out1,  # static in prebuilt non-gc
    ...
    def collect_roots(self):
        ...
        self.root_walker.walk_roots(
            MMTKMiniMarkGC._collect_ref_stk, # stack roots
            MMTKMiniMarkGC._collect_ref_stk, # static in prebuilt non-gc structures
