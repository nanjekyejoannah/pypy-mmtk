...
unsafe impl Send for PyPyEdge {}

impl Edge for PyPyEdge {
    fn load(&self) -> ObjectReference {
        match self {
            PyPyEdge::Simple(e) => e.load(),
            #[cfg(target_pointer_width = "64")]
            PyPyEdge::Compressed(e) => e.load(),
            PyPyEdge::Offset(e) => e.load(),
            PyPyEdge::Tagged(e) => e.load(),
        }
    }

    fn store(&self, object: ObjectReference) {
        match self {
            PyPyEdge::Simple(e) => e.store(object),
            #[cfg(target_pointer_width = "64")]
            PyPyEdge::Compressed(e) => e.store(object),
            PyPyEdge::Offset(e) => e.store(object),
            PyPyEdge::Tagged(e) => e.store(object),
        }
    }
}

#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct PyPyMemorySlice(*mut [ObjectReference]);

unsafe impl Send for PyPyMemorySlice {}

impl MemorySlice for PyPyMemorySlice {
    type Edge = PyPyEdge;
    type EdgeIterator = PyPyMemorySliceIterator;

    fn iter_edges(&self) -> Self::EdgeIterator {
        PyPyMemorySliceIterator {
...
pub struct PyPyMemorySliceIterator {
    cursor: *mut ObjectReference,
    limit: *mut ObjectReference,
}

impl Iterator for PyPyMemorySliceIterator {
    type Item = PyPyEdge;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.cursor >= self.limit {
            ...
            Some(PyPyEdge::Simple(SimpleEdge::from_address(
...
impl TaggedEdge {
    // The PyPy has OBJECT_REF_OFFSET = 4.
